...
include get_template_directory().'/landing-page/functions.php';

include get_template_directory().'/bezpieczna-firma-z-avg/bezpieczna-firma-z-avg.php';

add_action( 'after_body', 'ga' );
function ga() {
        ?>
        <script>
...
include get_template_directory().'/polls/polls-post-type.php';

add_action('wp_enqueue_scripts', 'register_custom_css');
function register_custom_css() {
  wp_register_style('gr-form', get_template_directory_uri().'/css-custom/gr-form.css', false, false, 'all');
  wp_enqueue_style('gr-form');
}
// TODO: przeniesc deaktywacje ogolnych CSSow do php z funkcjami ankiet
add_action('wp_enqueue_scripts', 'deregister_css_when_polls',11);
function deregister_css_when_polls() {
  $excluded = array(
    'polls',
    'bonus'
  );
  if (in_array(get_post_type(), $excluded)) {
          wp_dequeue_style('tryb-box-style');
          wp_deregister_style('gr-form');
          wp_dequeue_style('smartmag-core');
  }
}
...
$ad_code = '<div><script data-ad-client="ca-pub-7062317950964139" async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js"></script></div>';
...
include get_template_directory().'/strona-bonusy/page-bonus-post-type.php';
